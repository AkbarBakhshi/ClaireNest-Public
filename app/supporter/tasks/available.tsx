...
export default function AvailableTasksScreen() {
  ...
  // Get available tasks (not overdue, open)
  const availableTasks = requests
    .filter((task) => task.status === "open" && !isOverdue(task))
    .sort(
      (a, b) =>
  ...
  return (
    <SafeAreaView className="flex-1 bg-background px-4">
      ...
      <View className="flex-row items-center mt-4 mb-6">
        <Button variant="ghost" className="p-0 mr-2" onPress={() => router.back()}>
      ...
      <Animated.View className="flex-1" entering={FadeInUp.duration(300)}>
        {loading ? (
          ...
          <View className="flex-1 justify-center items-center">
            ...
            <Text className="text-center text-muted-foreground mt-2 mb-4">
