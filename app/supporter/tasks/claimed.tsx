...
export default function ClaimedTasksScreen() {
  ...
  // Get claimed tasks for the current user
  const claimedTasks = requests
    .filter(
      (task) =>
  ...
  // Count overdue claimed tasks
  const overdueClaimedTasksCount = claimedTasks.filter((task) => {
        ...
        {loading ? (
          ...
          <View>
            {overdueClaimedTasksCount > 0 && (
              <View className="my-4 bg-amber-500/10 p-3 rounded-lg">
                <Text className="text-amber-600 text-center">
                  You have {overdueClaimedTasksCount} overdue task
                  {overdueClaimedTasksCount > 1 ? "s" : ""} that need attention
                </Text>
              </View>
            )}
            <TaskList
