...
const Card = React.forwardRef<ViewRef, ViewProps>(({ className, ...props }, ref) => (
  <View
    ref={ref}
    className={cn(
      'rounded-[12px] border-2 border-border bg-card shadow-sm shadow-foreground/10',
      className
    )}
    {...props}
  />
...
const CardDescription = React.forwardRef<TextRef, TextProps>(({ className, ...props }, ref) => (
  <Text style={{ fontFamily: "Quicksand_400Regular" }} ref={ref} className={cn('text-sm text-muted-foreground', className)} {...props} />
));
