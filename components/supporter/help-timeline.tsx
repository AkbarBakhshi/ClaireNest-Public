...
import { HelpRequest, Message, UserData } from "@/interfaces";
...
interface HelpTimelineProps {
  request: HelpRequest;
  parentUser?: UserData | null;
}

export default function SupporterHelpTimeline({
  request,
  parentUser,
}: HelpTimelineProps) {
  ...
  const getParentName = () => {
    if (!parentUser) return "Parent";
    return parentUser.firstName
      ? `${parentUser.firstName}${
          parentUser.lastName
            ? ` ${
                parentUser.lastName.charAt(0).toUpperCase() +
                parentUser.lastName.slice(1)
              }`
            : ""
        }`
      : "Parent";
  ...
  const generateTimelineEvents = useCallback(() => {
    ...
    events.push({
      ...
      description: `${request.title} request was created by ${getParentName()}`,
    ...
    if (request.updateHistory) {
      request.updateHistory.forEach((update) => {
        ...
        switch (update.type) {
          case "claimed":
            const isCurrentUserClaimer = update.userId === userData?.userId;
            const claimerName = isCurrentUserClaimer
              ? "You"
              : "Another supporter";
            title = "Request Claimed";
            description = `${claimerName} offered to help with this request`;
            iconColor = "#f59e0b"; // Amber color
            break;
          case "abandoned":
            const isCurrentUserAbandoner = update.userId === userData?.userId;
            const abandonerName = isCurrentUserAbandoner
              ? "You have"
              : "Another supporter has";
            title = "Request Abandoned";
            description = `${abandonerName} abandoned this request`;
    ...
    messages.forEach((message: Message) => {
      ...
      const displayName = isSentByUser
        ? "You"
        : message.senderId === request.parentId
        ? getParentName()
        : "Another supporter";
  ...
  }, [request, messages, userData?.userId, parentUser]);
  ...
  // Only allow chat if request is claimed by the current user
  const isChatEnabled =
    request.status === "claimed" && request.claimedBy === userData?.userId;
  ...
  return (
    <View className="flex-1">
      ...
      {/* Chat input - only shown if claimed by the current user */}
      {isChatEnabled && (
        <RNAnimated.View
          style={{
            transform: [
